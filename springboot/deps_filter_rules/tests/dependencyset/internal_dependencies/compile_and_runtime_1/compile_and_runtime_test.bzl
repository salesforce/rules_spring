...
def _test_no_filtering(ctx):
   ...
   B (c)  E (r)   /
    ...
    for jar_field in jar_fields:
        ...
        update_jar_name("libno_filtering_test_lib", "libbase_lib", deps_filter_jars_list)
...
def _test_filtered_deps(ctx):
   ...
   B (c)  E (r)   /
    ...
    Deps Dependencyset Input:
        items = [":lib_a"]

    Runtime Dependencyset Input:
        items = [":lib_i"]

    Dependencyset exclusions Input:
        deps_exclude_labels = [":lib_b", ":lib_g"]
        deps_exclude_paths = []
        exclude_transitives = False

    Expected Output:
        JARs preserved: test_lib, A, I, E, F, C, D
        JARs excluded: B, G (by label, direct deps only, transitives preserved)
        
        Compile-time JARs: test_lib-hjar, A-hjar, C-hjar
        Runtime JARs: test_lib, A, I, E, F, C, D (all preserved deps + transitives)
    ...
    expected_compile_jars = [
        "libexclude_b_g_test_lib-hjar.jar",
    ]

    expected_full_compile_jars = [
        "libexclude_b_g_test_lib.jar",
    ]
...
def _test_filtered_deps_exclude_transitive(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        JARs preserved: test_lib, A, I, E
        JARs excluded: B, G (by label) and all their transitives
        
        Compile-time JARs: test_lib-hjar, A-hjar
        Runtime JARs: test_lib, A, I, E (only preserved deps, no transitives of excluded deps)
    ...
    expected_compile_jars = [
        "libexclude_b_g_transitives_test_lib-hjar.jar",
    ]

    expected_full_compile_jars = [
        "libexclude_b_g_transitives_test_lib.jar",
    ]
...
def _test_path_based_exclusions(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        JARs preserved: test_lib, A, I, E, C, D, G
        JARs excluded: B, F (by path pattern match, transitives may be preserved)
        
        Compile-time JARs: test_lib-hjar, A-hjar, C-hjar
        Runtime JARs: test_lib, A, I, E, C, D, G (all preserved deps + transitives not matching exclusion path patterns)
    ...
    expected_compile_jars = [
        "libpath_based_exclusions_test_lib-hjar.jar",
    ]

    expected_full_compile_jars = [
        "libpath_based_exclusions_test_lib.jar",
    ]
...
def _test_interface_implementation_jars(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        compile_jars: libinterface_impl_test_lib-hjar.jar
        full_compile_jars: libinterface_impl_test_lib.jar
    """
    env = unittest.begin(ctx)
    javainfo_filtered = ctx.attr.test_lib[JavaInfo]
    ...
    expected_compile_jars = ["libinterface_impl_test_lib-hjar.jar"]
    expected_full_compile_jars = ["libinterface_impl_test_lib.jar"]
...
def _test_multiple_exclusions_with_transitive(ctx):
   ...
   B (c)  E (r)   /
    ...
    expected_compile_jars = [
        "libmultiple_exclusions_with_transitive_test_lib-hjar.jar",
    ]

    expected_full_compile_jars = [
        "libmultiple_exclusions_with_transitive_test_lib.jar",
    ]
...
def _test_multiple_exclusions_without_transitive(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        ...
        Compile-time JARs: test_lib-hjar, A-hjar, C-hjar
        Runtime JARs: test_lib, A, I, E, F, C, D
    ...
    expected_compile_jars = [
        "libmultiple_exclusions_without_transitive_test_lib-hjar.jar",
    ]

    expected_full_compile_jars = [
        "libmultiple_exclusions_without_transitive_test_lib.jar",
    ]
...
def _test_empty_exclusion_lists(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        JARs preserved: test_lib, A, I, B, C, D, E, F, G
        
        Compile-time JARs: test_lib-hjar, A-hjar, B-hjar, C-hjar
        Runtime JARs: test_lib, A, I, B, C, D, E, F, G
    ...
    expected_compile_jars = [
        "libempty_exclusion_lists_test_lib-hjar.jar",
    ]

    expected_full_compile_jars = [
        "libempty_exclusion_lists_test_lib.jar",
    ]
...
def _test_multiple_paths(ctx):
   ...
   B (c)  E (r)   /
    ...
    expected_compile_jars = [
        "libmultiple_paths_test_lib-hjar.jar",
    ]

    expected_full_compile_jars = [
        "libmultiple_paths_test_lib.jar",
    ]
