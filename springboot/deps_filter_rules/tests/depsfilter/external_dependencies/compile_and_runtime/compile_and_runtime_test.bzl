...
def _test_no_filtering(ctx):
    ...
    expected = compute_expected_jars(ctx)
...
def _test_runtime_deps_only(ctx):
    ...
    # Updating manually because the output_jar for 'runtime_deps' is a full_compile_jar if no 'deps' specified
    expected_full_compile_jars = ["processed_spring-boot-starter-oauth2-client-3.3.11.jar"]
...
def _test_single_runtime_dep_with_exclusions(ctx):
    ...
    expected["full_compile_jars"] = ["processed_spring-boot-starter-oauth2-client-3.3.11.jar"]
