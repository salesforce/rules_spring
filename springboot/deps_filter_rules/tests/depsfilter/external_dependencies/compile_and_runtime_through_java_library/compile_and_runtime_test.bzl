...
def _test_no_filtering(ctx):
    ...
    test_lib_name = "base_lib_with_deps_and_runtime_deps"
...
def _test_runtime_deps_only_A(ctx):
    """
    ...
    test_lib_name = "base_lib_with_runtime_deps"
    expected = compute_expected_jars(ctx, test_lib_name)
    ...
    # Runtime deps should not contribute to compile-time JARs
    compile_jars = [jar.basename for jar in javainfo_filtered.compile_jars.to_list()]
    full_compile_jars = [jar.basename for jar in javainfo_filtered.full_compile_jars.to_list()]
    
    asserts.equals(env, len(compile_jars), 0, 
        "Expected no compile JARs when only runtime_deps provided, got: {}".format(len(compile_jars)))

    # Note: full_compile_jars is not empty because the output_jar is a full_compile_jar
    asserts.equals(env, len(full_compile_jars), 1, 
        "Expected no full compile JARs when only runtime_deps provided, got: {}".format(len(full_compile_jars)))
...
runtime_deps_only_A_test = unittest.make(_test_runtime_deps_only_A, attrs = {
    "filtered": attr.label(providers = [JavaInfo]),
    "deps_labels": attr.label_list(
        default = [],
    ),
...
def _test_runtime_deps_only_B(ctx):
    """
    Tests behavior when only runtime_deps are provided (no deps).
    """
    env = unittest.begin(ctx)
...
runtime_deps_only_B_test = unittest.make(_test_runtime_deps_only_B, attrs = {
    "filtered": attr.label(providers = [JavaInfo]),
    "deps_labels": attr.label_list(
        default = [],
    ),
...
def _test_single_runtime_dep_with_exclusion_A(ctx):
    """
    ...
    test_lib_name = "base_lib_with_single_runtime_dep"
    expected = compute_expected_jars(ctx, test_lib_name)
...
single_runtime_dep_with_exclusions_A_test = unittest.make(_test_single_runtime_dep_with_exclusion_A, attrs = {
    "filtered": attr.label(providers = [JavaInfo]),
    "deps_labels": attr.label_list(
        default = [],
    ),
...
def _test_single_runtime_dep_with_exclusions_B(ctx):
    """
    Tests behavior with a single runtime dependency and exclusions.
    """
    env = unittest.begin(ctx)
...
single_runtime_dep_with_exclusions_B_test = unittest.make(_test_single_runtime_dep_with_exclusions_B, attrs = {
    "filtered": attr.label(providers = [JavaInfo]),
    "deps_labels": attr.label_list(
        default = [],
    ),
...
def _test_compile_deps_only(ctx):
    ...
    test_lib_name = "base_lib_with_deps"
    expected = compute_expected_jars(ctx, test_lib_name)
...
def _test_single_dep_with_exclusions(ctx):
    ...
    test_lib_name = "base_lib_with_single_dep"
    expected = compute_expected_jars(ctx, test_lib_name)
...
def _test_one_compile_one_runtime_dep(ctx):
    ...
    test_lib_name = "base_lib_with_single_dep_and_runtime_dep"
    expected = compute_expected_jars(ctx, test_lib_name)
