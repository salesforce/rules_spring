...
def _test_filtered_deps(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        JARs preserved: base_lib, A, E, F, C, D
        JARs excluded: B, G (by label, direct deps only, transitives preserved)
        
        Compile-time JARs: base_lib-hjar, A-hjar, C-hjar
        Runtime JARs: base_lib, A, E, F, C, D (all preserved deps + transitives)
...
def _test_filtered_deps_exclude_transitive(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        JARs preserved: base_lib, A, E
        JARs excluded: B, G (by label) and all their transitives
        
        Compile-time JARs: base_lib-hjar, A-hjar
        Runtime JARs: base_lib, A, E (only preserved deps, no transitives of excluded deps)
...
def _test_path_based_exclusions(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        JARs preserved: base_lib, A, E, C, D, G
        JARs excluded: B, F (by path pattern match, transitives may be preserved)
        
        Compile-time JARs: base_lib-hjar, A-hjar, C-hjar
        Runtime JARs: base_lib, A, E, C, D, G (all preserved deps + transitives not matching exclusion path patterns)
...
def _test_interface_implementation_jars(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        compile_jars: libbase_lib-hjar.jar
        full_compile_jars: libbase_lib.jar
    """
    env = unittest.begin(ctx)
    javainfo_filtered = ctx.attr.filtered[JavaInfo]
...
def _test_multiple_exclusions_without_transitive(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        ...
        Compile-time JARs: base_lib-hjar, A-hjar, C-hjar
        Runtime JARs: base_lib, A, E, F, C, D
...
def _test_empty_exclusion_lists(ctx):
   ...
   B (c)  E (r)   /
    ...
    Expected Output:
        JARs preserved: base_lib, A, B, C, D, E, F, G
        
        Compile-time JARs: base_lib-hjar, A-hjar, B-hjar, C-hjar
        Runtime JARs: base_lib, A, B, C, D, E, F, G
